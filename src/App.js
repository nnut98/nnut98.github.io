...
import { Beer, Droplet, Wheat, Award, Zap, RefreshCw, ShoppingBag, X, User, Star, Utensils, LogIn, LogOut, Trash2, ChevronRight, MapPin, CreditCard, Check, LayoutGrid, Sun, Moon, Flower2 } from 'lucide-react';
...
<div className="flex gap-3 overflow-x-auto pb-4 snap-x hide-scrollbar px-1 pt-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
        {randomSelection.map((b) => (
          ...
          >
            <div className={`w-16 h-20 rounded-xl flex items-center justify-center bg-slate-50 border-2 transition-all ${selectedId === b.id ? 'border-[#003A5D] bg-white shadow-md ring-2 ring-[#003A5D]/10' : 'border-slate-200 group-hover:border-slate-300'}`}>
